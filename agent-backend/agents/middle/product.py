...
4. 收集底层发现，调 LLM 综合分析
5. 填充 ProductDesignState 的 Public 字段
6. 打印中间过程和最终输出
...
使用示例：
    ...
    leader = ProductLeader(llm, search)
    state = await leader.run(project_summary="宠物社交App", focus_areas=["功能设计", "MVP范围"])
    # state.key_points 是整理好的产品设计要点
"""
...
class ProductLeader:
    ...
    async def run(
        ...
        5. 填充 ProductDesignState

        参数：
            project_summary：项目描述摘要（来自顶层 Agent）
            focus_areas：关注维度列表，如 ["功能设计", "MVP范围", "用户体验"]
