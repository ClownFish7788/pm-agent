...
from prompts.templates import build_change_leader_prompt, build_review_prompt
from schemas import (
    AgentStatus,
    ChangeState,
    DepartmentTask,
...
class ChangeLeader:
    ...
    管理底层搜索 Agent，审核报告质量，综合启动数据为行动清单。
    关注"今天该干什么"——最务实的中层。
    ...
    async def run(
        ...
        for cycle in range(1, max_cycles + 1):
            ...
            print(f"  ⚡ [ChangeLeader] 第 {cycle}/{max_cycles} 轮审核 — "
    ...
    async def _review_sub_agents(
        ...
        try:
            ...
            log_error("ChangeLeader.review", f"审核调用失败: {type(e).__name__}: {e}")
            return [
                SubAgentReview(
                    sub_id=info["sub_id"],
    ...
    def _generate_search_queries(
        ...
        """根据 Top Agent 下发的 core_topic + focus_areas 生成启动策略搜索关键词。"""
        queries: list[str] = []
        if task.core_topic:
