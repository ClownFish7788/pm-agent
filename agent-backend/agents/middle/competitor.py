...
5. 收集底层发现，调 LLM 综合分析
6. 填充 CompetitorState 的 Public 字段
7. 打印中间过程和最终输出
...
class CompetitorLeader:
    ...
    管理底层搜索 Agent，审核报告质量，综合多源发现为竞品分析报告。
    ...
    async def run(
        ...
        for cycle in range(1, max_cycles + 1):
            ...
            if all_passed:
                print(f"  🏢 [CompetitorLeader] 全部通过审核 ✅")
                break
    ...
    async def _review_sub_agents(
        ...
        try:
            ...
            log_error("CompetitorLeader.review", f"审核调用失败: {type(e).__name__}: {e}")
            return [
                SubAgentReview(
                    sub_id=info["sub_id"],
